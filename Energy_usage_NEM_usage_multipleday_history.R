...
df_usage_day <- filtered_group_E1 %>%
  ...
  mutate(usage_value = as.numeric(usage_value)) 
...
# plot 
ggplot(df_usage_day, aes(x = time_slot_numeric, y = usage_value, group = 1)) +
  geom_line(color = "blue", size = 1) +
  labs(
    title = "Usage from Grid Across Time Slots",
    x = "Time Slot",
    y = "Usage Value (kWh)"
  ) +
  theme_minimal() +
  ...
  scale_x_continuous(
    breaks = seq(0, max(df_usage_day$time_slot_numeric), by = 30),  # Show every 30th minute (or adjust as needed)
