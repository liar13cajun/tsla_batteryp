...
df_NEM_gen <- read.xlsx("C:/Project_R/Tesla_battery/Tesla_battery/Nem_gen_July.xlsx",check.names = TRUE)
df_NEM_supply <- read.xlsx("C:/Project_R/Tesla_battery/Tesla_battery/Nem_supply_July.xlsx",check.names = TRUE)
...
### change here
df_join <- merge(tsla_240716,df_xx)
...
ggplot()+
  ...
  labs(x = "time", y = "kWh", title = "2024-07-16")
#######################################################
#tsla home
tsla_home <- sum(tsla_240716$Home..kW.)
tsla_powerwall <- sum(tsla_240716$Powerwall..kW.)
tsla_solar <- sum(tsla_240716$Solar..kW.)
tsla_grid <- sum(tsla_240716$Grid..kW.)
